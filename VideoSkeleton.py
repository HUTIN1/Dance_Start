...
def combineTwoImages(image1, image2):
    print(f'type image1 {type(image1), image1}')
...
class VideoSkeleton:
    ...
    def __init__(self, filename, forceCompute=False, modFrame=10):
        ...
        print("directory: "+os.path.join(filename_dir,filename_base))
        if not os.path.exists(os.path.join(filename_dir,filename_base)):
            print("create directory: "+os.path.join(filename_dir,filename_base))
            os.makedirs(os.path.join(filename_dir,filename_base))
        ...
        for i in range(video.getTotalFrames()):
            ...
            if (i%mod_frame == 0):
                ...
                if isSke:
                    filename_im = os.path.join(filename_base ,"image" + str(i) + ".jpg")
                    filename_imsave = os.path.join(filename_dir ,filename_im)
